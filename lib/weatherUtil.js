...
function weatherResponseCallback(callback, error, weatherResponse, condition, city, state, date, WU_API_KEY, debug) {
  ...
  if (error) {
    ...
    if (weatherResponse.status == 'successful') {
      ...
      var full_state = locationUtil.getStateName(state);
      if (state && !city.includes(full_state)) weatherReply += ", " + full_state;
      ...
      switch (condition) {
        ...
        case 'Rain':
          if (weatherResponse.current) {
            var rain_inches = weatherResponse.current.precip_1hr_in;
      ...
      var options = state_names.concat(state_abbrevs);
      for(var i=0;i<options.length;i++) options[i]=options[i].toLowerCase();
      callback({
        ask: ["I found " + state_names.length + " states in the U.S. with a city named " + city + ":", state_string, "Which state are you interested in?"],
        options: options
      });
    } else {
