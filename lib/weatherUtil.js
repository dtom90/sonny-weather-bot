    ...
    makeWeatherRequest: function(WU_API_KEY, condition, city, state, date, debug, callback) {
      ...
      if (!city) {
        ...
        if (dateObject.error) weatherResponseCallback(callback, new Error(dateObject.error));
          ...
          let prefix = endpoint + dateObject.dataFeature;
          let query = '/q/';
          if (state) query += state + '/';
          query += city_param + '.json';
          let url = prefix + query;

          requestWeather(url, prefix, condition, city, state, dateObject, WU_API_KEY, debug, callback);
        }
      }
    }
  };
...
function requestWeather(url, prefix, condition, city, state, dateObject, WU_API_KEY, debug, callback){
  ...
  http.get(url, function(response) {
    ...
    response.on('end', function() {
      ...
      let interpretation = interpretResponse(weatherResponseObject, city, prefix);
      if (interpretation.error) weatherResponseCallback(callback, new Error(interpretation.error));
      else if (interpretation.disambiguate){
        url = prefix + interpretation.options[0]['l'] + '.json';
        requestWeather(url, prefix, condition, city, state, dateObject, WU_API_KEY, debug, callback);
      }
  ...
  if (weatherResponseObject.response.error) {
    ...
    if (weatherResponseObject['current_observation']) {
      ...
      for (let o in options) {
        if (!countries.includes(options[o]['country'])) countries.push(options[o]['country']);
        states.push(options[o]['state']);
      }
      if (countries.length == 1) {
        // Check to see if the states are all numbers
        let redundant = true;
        for (let s in states){
          if(isNaN(parseInt(states[s]))){
            redundant = false;
          }
        }
        if(redundant){
          ...
          let country_name = options[0]['country_name'];
          return {
  ...
  if (error) {
    ...
    if (weatherResponse.status == 'successful') {
      ...
      case 'Wind':
        ...
        else weatherReply += 'from the ' + SPOKEN_CARDINAL_POINT[forecast['avewind'].dir] +
            ' at ' + forecast['avewind']['mph'] + ' gusting to ' + forecast['maxwind']['mph'] + ' mph.';
        break;
      case 'Humidity':
      ...
      callback({
        tell: weatherReply
        //TODO image: weatherResponse.current ? weatherResponse.current.icon_url : forecast.icon_url
      }, city);
      ...
      weatherResponse.options.forEach(function(city) { if (!country_abbrevs.includes(city['country'])) {
