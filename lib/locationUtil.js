...
let locationUtil = (function() {
  ...
  return {
    ...
    getProbableCity: function(state) {

      if(state){
        if(state in STATE_ABBREVS_TO_NAMES)
          state = STATE_ABBREVS_TO_NAMES[STATE_ABBREVS_TO_NAMES];

        if(state in STATE_BIGGEST_CITY)
          return STATE_BIGGEST_CITY[state];
      }
      return null;
    },
    ...
    getProbableState: function(city, state) {
      ...
      if (state) return state.replace(/ /g, "_");

      // if (state && stateAbbrev(state) in STATE_ABBREVS_TO_NAMES) return state;

      let properCity = city.toProperCase();
    ...
    getStateName: function(str) {
      ...
      else if(str.toProperCase()==='District of Columbia') return 'D.C.';
